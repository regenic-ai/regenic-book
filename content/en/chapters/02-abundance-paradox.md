...
Status: Completed (v1.5, 2026-08-04, about 8500 words; acceptance passed 2026-07-25; v1.2 forward echo of market-side scissor gap; v1.3 five-gene root paragraph; v1.4 aside scaffolding cleanup; v1.5 prose-standard fluency pass; English terminology sync (principal-leader), open to ongoing public revision)
...
The reason is a plain systems law: **the chain moves at the speed of its slowest link, and execution is often no longer the slowest link.** Value creation is a chain: decide what to do → align understanding → execute → revise from feedback. AI makes the third link ten times faster; if links one, two, and four stay put, end-to-end speed barely budges—like raising one highway segment's limit from 120 to 1,200 while you sit at the toll booth.
...
Building an AI-native organization is not bolting new tools onto the old one—Klarna and MD Anderson had tools galore—but completing an **organizational gene evolution**: replacing the five old genes one by one.
...
Everett Rogers's diffusion-of-innovation theory sorts adopters into five groups: innovators (~2.5%) → early adopters (~13.5%) → early majority (~34%) → late majority (~34%) → laggards (~16%). Geoffrey Moore applied this to high-tech markets and located the critical break between early adopters and early majority—the original **chasm**. The break happens because **early adopters buy vision; the early majority buys certainty—and those two consensuses do not bridge on their own** (proportions are theoretical approximations, not fixed market quotas; classify users around the specific innovation and context).
...
**First, this chapter is not anti-tool.** "The gap is organizational" does not mean vendor choice doesn't matter or that the worst model will do. Tools set the ceiling; the organization sets what fraction you reach. For most organizations today the bottleneck is the latter—not license to "don't buy tools yet."
...
**Fourth, the five genes are not moral indictments.** Cognitive inertia is the brain's energy budget; information hoarding can be individually rational; execution worship was virtue when execution was expensive. They are not evil—obsolete: optimal for a dead cost structure. Insults don't transform organizations; changing the environment that makes these behaviors individually optimal does.
...
3. **Grade**: Compare the three answers. Three different standards—or no answer at all—and you are at the chasm's edge: tools amplifying three contradictory judgments.
...
6. Fluency ✓: English terminology sync (principal-leader); per `prose-standard.md`, cleared tour-guide asides, methodological demo footnotes, and mid-argument chapter/title self-reference; sources moved to in-text author–year.
